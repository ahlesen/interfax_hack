...
stop_words = stop_words + ['URL','USER','HASHTAG','ap','ар', '2ар', 'interfax', 'ru']
...
def check_max_tfidf(mega_title, df_val, target_title,col_print, gram_range = (1,3), limit_string=30):
    ...
    # пока так далее лучше ранжировать в зависимости от нграмм и символов до 30  
    ...
    while flag_2_next_max_value:
        ...
        ## мб стоит ещё сортировать от максимального количества (мб минимального) символов
...
def main(file_name_in:str, string_limit:int, file_name_out:str,):
    ...
    dataset = dataset[['title','mega_title','text']]
    ...
    preds_baseline = make_predictions_tfidf(dataset,target_title='target_text',limit_string=string_limit)
