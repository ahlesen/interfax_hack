...
def merge_text(dataset:pd.DataFrame):
    ...
    for story in tqdm(set(dataset['num_story'])):
...
def main(file_name_in:str, file_name_out:str,):
    ...
    # собираем заголовки по сюжетам в 1 строку
    ...
    model_load = T5Model("mt5", "mT5_checkpoint", args=model_args, use_cuda=False)
    ...
    preds_mT5 = model_load.predict(to_predict_train)
    print(len(preds_mT5))
    with open(file_name_out, 'w+') as fout:
        for preds in preds_mT5: 
            fout.write(preds+'\n')
    return preds_mT5 
